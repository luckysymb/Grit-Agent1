...
import type {
	StopReason as AnthropicSdkStopReason,
	Tool as AnthropicSdkTool,
...
/** Client instance type (default import path resolves as a non-constructable module under tsgo). */
...
): { client: AnthropicClient; isOAuthToken: boolean } {
...
function convertTools(tools: Tool[], isOAuthToken: boolean): AnthropicSdkTool[] {
	if (!tools) return [];
...
function mapStopReason(reason: AnthropicSdkStopReason | string): StopReason {
	switch (reason) {
		case "end_turn":
			return "stop";
		case "max_tokens":
