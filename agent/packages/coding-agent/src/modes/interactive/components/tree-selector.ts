...
class TreeList implements Component {
	...
	private formatToolCall(name: string, args: Record<string, unknown>): string {
		...
		switch (name) {
			case "read_file": {
				const path = shortenPath(String(args.target_file || ""));
				const entire = args.should_read_entire_file ? " (entire)" : "";
				return `[read_file: ${path}${entire}]`;
			}
			case "edit_file": {
				const path = shortenPath(String(args.target_file || ""));
			...
			case "grep_search": {
				...
				return `[grep_search: /${pattern.slice(0, 40)}/]`;
			}
			case "codebase_search": {
				const q = String(args.query || "").slice(0, 50);
				return `[codebase_search: ${q}${String(args.query || "").length > 50 ? "…" : ""}]`;
			}
			case "list_dir": {
				const path = shortenPath(String(args.relative_workspace_path || "."));
				return `[list_dir: ${path}]`;
			}
			case "file_search": {
				const q = String(args.query || "");
				return `[file_search: ${q}]`;
			}
			case "run_terminal_cmd": {
				const rawCmd = String(args.command || "");
				const cmd = rawCmd.replace(/[\n\t]/g, " ").trim().slice(0, 50);
				return `[run_terminal_cmd: ${cmd}${rawCmd.length > 50 ? "…" : ""}]`;
			}
			case "delete_file": {
				const path = shortenPath(String(args.target_file || ""));
			...
			case "edit_notebook": {
				const path = shortenPath(String(args.target_notebook || ""));
				return `[edit_notebook: ${path} @${args.cell_idx ?? "?"}]`;
			}
			case "reapply": {
				const path = shortenPath(String(args.target_file || ""));
				return `[reapply: ${path}]`;
